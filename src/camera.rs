...
fn create_view_rotation(eye: Vec3, target: Vec3, up: Vec3) -> Quat {
    ...
    let rotation_mat = Mat4::look_at_rh(Vec3::zero(), dir, up.normalize()).inverse();
...
mod tests {
    ...
    #[test]
    fn transform_ray_translate_glam() {
        let camera = CameraG::new()
    ...
    #[test]
    fn pixel_ray_glam() {
        let camera = CameraG::new().set_fov(FRAC_PI_2).set_dimensions(400, 200);
    ...
    #[test]
    fn pixel_ray_with_transform_glam() {
        let camera = CameraG::new()
            .set_fov(FRAC_PI_2)
            .set_dimensions(400, 200)
            .set_eye(Vec3::new(1., 2., -3.))
